...
func _enter():
	...
	SceneManager.update_progress.connect(get_root().gui_progress._on_progress_changed)
...
func _exit():
	...
	SceneManager.update_progress.disconnect(get_root().gui_progress._on_progress_changed)
	
	
func _update(_p_delta):
		if scene_loaded:
			scene_loaded = false
			if get_root().gui_hud == null or not is_instance_valid(get_root().gui_hud):
				var gui_hud_id = GuiManager.add_gui("hud", 0, null)
				get_root().gui_hud = GuiManager.get_gui(gui_hud_id)
				
			get_root().gui_hud.ui_inventory.inventory_data = GameStateService.get_global_state_value("inventory_data")
			if scene.has_method("setup_hud"):
				scene.setup_hud(get_root().gui_hud)
