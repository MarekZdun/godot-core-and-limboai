...
func _enter():
	...
	get_root().gui_curtain = GuiManager.get_gui(gui_curtain_id)
	GuiManager.manager_gui_loaded.connect(_on_gui_on_screen)
	
	
func _exit():
	GuiManager.manager_gui_loaded.disconnect(_on_gui_on_screen)
	if is_instance_valid(get_root().gui_main_menu):
		GuiManager.destroy_gui(get_root().gui_main_menu.id, null)
...
func _on_gui_on_screen(gui):
	if gui.id == get_root().gui_curtain.id:
